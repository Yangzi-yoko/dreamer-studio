...
- Node.js 18（服务器为 18.19；本地如需安装请保持 18 系列）
...
describe('PermissionsGuard', () => {
  ...
  it('allows super admin without permission check', () => {
    ...
    expect(guard.canActivate(makeContext({ isSuper: true }, []))).toBe(true);
  });

  it('rejects non-super admin missing permission', () => {
    const reflector = { getAllAndOverride: jest.fn(() => ['system:admin:create']) };
    const service = { getPermissionCodes: jest.fn().mockResolvedValue(['system:role:list']) };
    const guard = new PermissionsGuard(reflector as any, service as any);
    expect(guard.canActivate(makeContext({ isSuper: false, adminId: 2 }, []))).rejects.toThrow(
...
注意：`AdminService` 与 `PermissionsGuard` 存在循环依赖，`system.module.ts` 使用 `forwardRef` 处理（见 Step 4）。
...
@Module({
  imports: [TypeOrmModule.forFeature([AdminUser, Role, Menu])],
...
说明：PermissionsGuard 在运行时注入 AdminService，两者都在同一模块内，无跨模块循环依赖；`forwardRef` 仅当控制器层出现循环时使用，本模块不需要。
...
同时 `AuthModule` 的 imports 需要引入 `SystemModule`（`forwardRef` 若出现循环则使用）：
...
async function seed(): Promise<void> {
  ...
  const menuDefs = [
    { title: '仪表盘', path: '/dashboard', type: 'menu', permissionCode: 'dashboard', sort: 1 },
    { title: '会员管理', path: '/member', type: 'dir', permissionCode: 'member', sort: 10 },
    { title: '租赁管理', path: '/rental', type: 'dir', permissionCode: 'rental', sort: 20 },
    { title: '系统管理', path: '/system', type: 'dir', permissionCode: 'system', sort: 99 },
    { title: '管理员管理', path: '/system/admin', type: 'menu', permissionCode: 'system:admin:list', sort: 1, parent: 'system' },
    { title: '新增管理员', path: '', type: 'button', permissionCode: 'system:admin:create', sort: 1, parent: 'system' },
    { title: '分配角色', path: '', type: 'button', permissionCode: 'system:admin:assign-role', sort: 2, parent: 'system' },
    { title: '删除管理员', path: '', type: 'button', permissionCode: 'system:admin:delete', sort: 3, parent: 'system' },
    { title: '角色管理', path: '/system/role', type: 'menu', permissionCode: 'system:role:list', sort: 2, parent: 'system' },
    { title: '角色授权', path: '', type: 'button', permissionCode: 'system:role:assign-menu', sort: 1, parent: 'system' },
    { title: '菜单管理', path: '/system/menu', type: 'menu', permissionCode: 'system:menu:list', sort: 3, parent: 'system' },
  ];

  const saved: Menu[] = [];
  for (const def of menuDefs) {
    const parent = def.parent ? saved.find((m) => m.title === def.parent) : undefined;
