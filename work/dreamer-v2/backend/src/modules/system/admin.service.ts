...
export class AdminService extends BaseService<AdminUser> {
  ...
  private toSafeAdmin(admin: AdminUser): SafeAdmin {
  ...
  async createAdmin(operator: { isSuper: boolean }, dto: CreateAdminDto): Promise<SafeAdmin> {
    ...
    return this.toSafeAdmin(await this.repo.save(admin));
  }

  async assignRoles(operator: { isSuper: boolean }, adminId: number, roleIds: number[]): Promise<SafeAdmin> {
    this.assertSuper(operator);
  ...
  async toggleStatus(operator: { isSuper: boolean }, id: number): Promise<SafeAdmin> {
    this.assertSuper(operator);
